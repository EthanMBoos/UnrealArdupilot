...
UnrealArdupilot is a vehicle-dynamics and controls simulator built around Unreal Engine and
ArduPilot SITL. The first version is a fixed-wing aircraft; boat support follows.
...
[UNREAL.md](docs/UNREAL.md) covers the Cesium map, georeference, and Unreal project setup.
[AERO.md](docs/AERO.md) covers the current fixed-wing work. [WATER.md](docs/WATER.md) covers the later
...
```text
core/       C++23 math, state, clock, aircraft model, sensors, config, logs and CLI integrator
...
struct AircraftModelOutput {
    WrenchAtComBody aero;
    WrenchAtComBody propulsion;
    AirData air_data;
};

AircraftModelOutput EvaluateAerosonde(
...
channel map, limits, valid range, sources, and hashes. `run.json` says what to do this time: which
...
Both use schema version 1. Unknown fields, duplicate keys, nonfinite values, bad ranges, unsupported
versions, and wrong hashes fail validation. Paths are relative to the file that contains them.
...
The manifest records the machine, toolchain, UE and ArduPilot versions, fixed clock, dependency and
input hashes, binary hashes, warnings, stop reason, metrics, and file hashes. It excludes itself from
its internal hash list. Release/reference runs need a clean full Git commit. Dirty runs are fine for
testing but are labelled `exploratory` and record the base commit and file inventory.
...
| A1-A4 | [aircraft model](docs/AERO.md) | equations, JSBSim comparison, trim and linearization pass |
| B1 | Linux/macOS UE setup | UE, native compiler, Docker networking, ports and dependencies pass |
| G0 | [Cesium groundwork](docs/UNREAL.md) | configured LLA, axes, height datum and setup failures pass |
...
Code and documentation authored for this repo use the [MIT License](LICENSE).

ArduPilot and the pinned BYU MAVSim repo are GPL-3.0. JSBSim is LGPL-2.1, while individual aircraft
files may have other notices. Keep these tools separately installed and out of Unreal packages until
the licensing has been reviewed. The relevant terms are the
[ArduPilot license](https://github.com/ArduPilot/ardupilot/blob/e0652af4c0c9657c04672ed3e21b71de75c74763/COPYING.txt),
[BYU license](https://github.com/byu-magicc/mavsim_public/blob/a8489bbc30ab9e746dccac8ac614ab2f8acac6bb/LICENSE),
[JSBSim license](https://raw.githubusercontent.com/JSBSim-Team/jsbsim/3b25f25e49b42d0489c04ac805674fc1450ca579/COPYING),
and the [Unreal EULA](https://www.unrealengine.com/eula/unreal).

The BYU-derived JSBSim test fixture stays outside Unreal packaging. [Third-party notices](THIRD_PARTY_NOTICES.md)
track that boundary. Cesium for Unreal is
[Apache-2.0](https://github.com/CesiumGS/cesium-unreal/blob/v2.28.0/LICENSE), and streamed terrain and
imagery keep their required credits through the Cesium credit display.
