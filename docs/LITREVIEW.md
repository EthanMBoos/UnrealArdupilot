...
| Unreal/Chaos open-loop runtime | Smoke and local U0/U1 suites implemented | Mechanics/timing, matched-state wrench, and trajectory-refinement probes pass locally; cross-platform evidence remains |
| Cesium runtime | Local G0 coordinate acceptance implemented | Real georeference placement, WGS84/MSL conversion, axes, heading, and wrench isolation pass on macOS; streamed terrain is optional visual work |
| ArduPlane JSON/UDP transport | Local U2 transport/fault and U3 FBWA suites implemented | Consecutive exchange, explicit transport failures, readiness-gated release, recovered air-start flight, and command replay pass locally; cross-platform evidence remains |
