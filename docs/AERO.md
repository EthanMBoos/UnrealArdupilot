...
let loads approach zero with speed.

The JSBSim comparison, trim, linearization, and replay must all pass. The same 10 s script runs at
`dt=1/120`, `1/240`, and `1/480`; named signals are compared at common times with scaled norms and an
absolute floor. The two finest runs should differ by less than 0.1%.

## Order

| Step | Work | Done when |
|---|---|---|
| A1 | parameters, pure aero and hand checks | signs, low-speed and high-angle safety checks pass |
| A2 | JSBSim comparison | coefficients and wrenches agree across the test range |
| A3 | propeller, ISA and trim | the operating point and 10 s drift check pass |
| A4 | linearization, scripted input and replay | matrices converge and local/nonlinear response agrees |

Unreal and ArduPilot work continues with U0-U3 in [README.md](../README.md). Flight-data validation,
better stall data, ground operation, richer propulsion, sensor errors, and more aircraft come later.
