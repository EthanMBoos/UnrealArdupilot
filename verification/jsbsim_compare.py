#!/usr/bin/env python3
"""Compare uvd's pure aero model with the pinned aero-only JSBSim fixture."""
...
def main():
    ...
    parser.add_argument("--aircraft", default="examples/aircraft/aerosonde.json")
    ...
    try:
        ...
        raise SystemExit("Install pinned JSBSim 1.3.1 before running this offline check") from exc
    version = getattr(jsbsim, "__version__", "unknown")
    if version != "1.3.1":
        raise SystemExit(f"Expected JSBSim 1.3.1, got {version}")
    ...
    process = subprocess.run([args.uvd, "model-probe", args.aircraft], input="".join(json.dumps(p)+"\n" for p in probes), text=True, capture_output=True, check=True)
    actual = [json.loads(line) for line in process.stdout.splitlines() if line]
    ...
    for index, (got, ref) in enumerate(zip(actual, references)):
        ...
        groups = (("air_data",got_air,ref["air_data"],1e-10,0.0),
                  ...
                  ("force_n",got["force_n"],ref["force_n"],1e-5,1e-8*qbar_s),
                  ("moment_nm",got["moment_nm"],ref["moment_nm"],1e-6,1e-8*qbar_s*max(SPAN,CHORD)))
